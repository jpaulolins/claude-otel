...
To register the server in Codex CLI, run:

```bash
codex mcp add
```

Or add it manually to `~/.codex/config.toml` (see `config-samples/codex/config.toml`).
...
Prompts are canned instructions that guide the agent to call the right tool and format
the output. Invoke them using your client's prompt/slash-command interface.
...
> **TODO:** Native OTEL telemetry export from Codex CLI is not currently documented.
> A future release of this project will ship a local interceptor binary (built from
> `go-hook-mcp-api/cmd/audit`) that acts as an HTTP proxy between the Codex CLI process
> and the OpenAI API, emitting OTLP events that mirror the hook payload schema used by
> Claude Code. See `docs/client-setup.md` for the roadmap.
